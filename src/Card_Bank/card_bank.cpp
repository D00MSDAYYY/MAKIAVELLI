...
Card
Card_Bank::card(Country* country)
{
    ...
    if(!_main_card_buffer.empty())
    //! maybe optimize this using if(!_main_card_buffer.empty() &&
    //! _main_card_buffer.size() != 1) and shuffle in another thread
    //! then returning last card
    {
