...
Server_Launch::Server_Launch(QWidget* parent)
    : QWidget(parent),
      ui(new Ui::Server_Launch)
{
    ...
    connect(ui->create_shutdown_button,
            &QPushButton::clicked,
            [this, server_creating, server_gui]()
            {
                ...
                else if(ui->stackedWidget->currentIndex() == server_gui_index)
                {
                    ...
                    thread_server_deleting = std::jthread{[this, server_gui]()
                    {
...
Server_Launch::~Server_Launch() { delete ui; }
