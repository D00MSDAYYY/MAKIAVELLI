...
Resources
Game_Factory::createResources(int index)
{
    if(res_ar.empty())
    {
        QFile res_file(":/json_files/resources.json");
        res_file.open(QIODevice::ReadOnly);
        //! if using GCC 13.2.0 QString::toStdString() cause compile error, with
        //! clang 16.0.6 OK
...
Points
Game_Factory::createPoints(int index)
{
    if(points_ar.empty())
    {
...
Locations
Game_Factory::createLocations(int index)
{
    if(loc_ar.empty())
    {
...
Cards_Holder
Game_Factory::createCardsHolder(int index)
{
    return Cards_Holder();
}

Activity_Points
Game_Factory::createActivityPoints(int index)
{
    return Activity_Points{3};
}

void
Game_Factory::createCardBank(std::unordered_map<uint32_t, Country>& pl)
{
    if(cards_ar.size() == 0)
    {
    ...
    for(auto& [ID, country]: pl) { country.cardsHolder().setDependices(_card_bank); }
}

void
Game_Factory::createMap(std::unordered_map<uint32_t, Country>& p)
{
    if(_map.get() == nullptr) { _map = std::shared_ptr<Map>(new Map{_num_of_players}); }
    ...
    for(auto& [ID, country]: p)
    {
        ...
        for(int in_x_row_count{0};
            _map->find(Cell_Type::COUNTRY_AREA, {x - 3, y - 3}, {x + 3, y + 3})
            || _map->find(Cell_Type::CAPITAL, {x - 3, y - 3}, {x + 3, y + 3});
            ++in_x_row_count)
        {
            ++x;
        ...
        std::uniform_int_distribution<int> di_country_size(MIN_COUNTRY_SIZE,
                                                           MAX_COUNTRY_SIZE);
        ...
        for(int i{1}; i < country_size && !border_line.empty(); ++i)
        {
            ...
            for(; !x_step && !y_step; x_step = di_step(dre), y_step = di_step(dre)) {}
            for(; _map->cell({border_cell_coord.first + x_step,
                              border_cell_coord.second + y_step})
                          .mapCellType()
                      == Cell_Type::COUNTRY_AREA
                  || _map->cell({border_cell_coord.first + x_step,
                                 border_cell_coord.second + y_step})
                             .mapCellType()
                         == Cell_Type::CAPITAL;)
            {
            ...
            std::pair<int, int> new_border_cell_coord{border_cell_coord.first + x_step,
                                                      border_cell_coord.second + y_step};
            _map->cell({new_border_cell_coord.first, new_border_cell_coord.second})
                .mapCellOwner(&(country.locations()));
            _map->cell({new_border_cell_coord.first, new_border_cell_coord.second})
                .mapCellType(Cell_Type::COUNTRY_AREA);
...
Game_Factory::Game_Factory(int numplay)
    : _num_of_players{numplay}
{
    init();
}

Game_Factory::~Game_Factory() { cleanUp(); }

std::unordered_map<uint32_t, Country>
Game_Factory::createPlayers()
{
    std::default_random_engine dre{
        uint32_t(std::chrono::system_clock::now().time_since_epoch().count())};
    ...
    createMap(players);
    createCardBank(players);  // TODO! change this logic and use this methods
                              // inside createLocations() and createCards()
